...
function aggregateData(data) {
  return data.reduce((monthlySums, entry) => {
    const dateObj = new Date(entry.date);
    const monthKey = format(dateObj, 'yyyy-MM');
    const hourKey = format(dateObj, 'HH');
    if (!monthlySums[monthKey]) {
      monthlySums[monthKey] = Array(24).fill(0); // Init array pour simplicité
    }
    const value = parseFloat(entry.value) || 0;
    monthlySums[monthKey][parseInt(hourKey)] += value;
    return monthlySums;
  }, {});
  // Pas d'arrondi ici ; faites-le côté client si besoin
}
